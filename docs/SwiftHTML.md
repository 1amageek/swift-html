...
SwiftHTML has three different concepts that must not be collapsed:

| Concept | Naming | Responsibility |
|---|---|---|
| Document | `HTMLDocument` | Complete document root with explicit `head` and `body` sections |
...
```mermaid
flowchart TB
    H["HTML"] --> I["HTMLDocument"]
    H --> J["Component"]
    I --> K["head + body"]
    J --> A["lowercase HTML tags"]
    J --> C["Component"]
    A --> B["DOM primitives"]
    C --> D["custom UI unit"]
...
An `HTMLDocument` forms the complete document root and cannot be nested.
Lowercase HTML structs and components are `Component` values that can appear
inside document sections and elements. Components form the semantic and
lifecycle boundary.

`HTML` is the common rendering and Sendable boundary:
...
SwiftHTML target is not the client runtime compilation unit. The pinned Swift
6.4 Embedded baseline supports `TaskLocal` and `Synchronization.Mutex`; the
remaining boundary is driven by the renderer, macro, reflection, and manifest
surface that a small browser runtime does not need:

| Full SwiftHTML feature | Embedded Swift constraint | Boundary decision |
|---|---|---|
| `Codable` JSON manifests and snapshots | `Codable`, `Decodable`, and `JSONEncoder` are unavailable | Keep JSON manifests on the server/dev/runtime bridge side |
| `TaskLocal` render context | Available in the pinned Swift 6.4 Embedded baseline | Keep the same scoped context contract across Native, WASM, and Embedded |
| `Synchronization.Mutex` | Available in the pinned Swift 6.4 Embedded WASM baseline | Protect shared mutable state with the same `Mutex<State>` API on every profile |
| `String(reflecting:)` and metatype identifiers | type reflection and metatypes are unavailable | Production client IDs must be generated by SwiftWeb, not reflected at runtime |
| `any HTML` and dynamic casts | existential values and dynamic casting are unavailable | Embedded rendering must use statically dispatched lowering |
...
The historical Swift 6.3.1 measurement below used no `wasm-opt`. Rerun the
measurement with the pinned Swift 6.4 snapshot before treating it as current:
...
## Stable Component Builder Output

Application code declares `content` as `some Component`. `@ComponentBuilder`
lowers every expression to `ComponentContent`, so a component boundary does
not expose or retain an expanding nested generic tuple type.
...
The authoring and lowering boundary is:

```text
some Component
  -> ComponentBuilder
      -> ComponentContent
          -> fragment node in HTMLGraph
```

`ComponentContent` is builder machinery. Authors compose `Component` values
and do not name `ComponentContent` in models, stored properties, or public API.
Fragment nodes belong to `HTMLGraph`; there is no fragment authoring protocol.

| Builder input | Builder output |
|---|---|
| No child | `ComponentContent` containing an empty fragment |
| One child | `ComponentContent` lowering the child |
| Multiple children | `ComponentContent` lowering an ordered fragment |
| Optional branch | `ComponentContent` preserving optional path identity |
| `if` / `switch` branch | `ComponentContent` preserving branch path identity |
| `for` output | `ComponentContent` preserving positional array identity |
...
The builder records optional, conditional, and array path segments while
lowering to `ComponentContent`; graph identity therefore remains explicit
without publishing recursive wrapper types.
...
```swift
public struct ForEach<Data, ID: Hashable & Sendable, Content: Component>: Component
...
SwiftHTML wraps component rendering in `Observation.withObservationTracking`. When an observable property that was read while building `content` changes, the owning component is marked dirty. Hydration can then rerender and patch that component boundary.
...
| Step | Rule |
|---|---|
| Build symbol graph | Treat component content, event handlers, state access, observable access, and helper functions as symbols |
...
| Builder | `ComponentContent` lowering and explicit control-flow components |
