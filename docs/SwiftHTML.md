...
Embedded Swift rejects several runtime features that the full renderer and
manifest layer intentionally use:
...
| `TaskLocal` render context | `@TaskLocal` is unavailable | Use a client runtime context that does not depend on task-local storage |
| `Synchronization.Mutex` | `Mutex` is unavailable in the embedded SDK | Use a single-threaded client runtime storage primitive |
